...
async function decoratorPositionRule(context) {
  const userOptions = context.options[0] || {};
  const filePath = context.getFilename();
  const prettierOptions = await lineLength(userOptions, filePath);
...
function placeDecoratorsBesideProperty(context, node, options) {
  for (const decoratorConfig of options.overrides[PREFER_INLINE]) {
    ...
    if (totalLineLength <= options.printWidth) {
...
async function lineLength(userOptions, filePath) {
  const eslint = new ESLint();
  let eslintPrettierOptions = {};

  if (!prettier) {
    prettier = require('prettier');
  }

  const configForFile = (await eslint.calculateConfigForFile(filePath)) || {};
  if (configForFile.rules) {
    eslintPrettierOptions = configForFile.rules['prettier/prettier'];
  }
  ...
  return { ...userOptions, ...prettierOptions.printWidth };
}
